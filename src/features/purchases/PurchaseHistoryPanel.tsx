...
export const PurchaseHistoryPanel: React.FC = () => {
  ...
  const fetchHistory = async () => {
    ...
    try {
      const data: PurchaseHistoryRow[] = await invoke('get_purchase_history');
      setHistory(data);
    } catch (err: any) {
      ...
      <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex flex-col md:flex-row gap-6 items-start md:items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-slate-800 flex items-center gap-2">
            <History className="h-6 w-6 text-blue-500" /> Purchase History
          </h2>
          <p className="text-sm text-slate-500 mt-1">Review past purchases, check statuses, and reprint invoices</p>
        </div>
                ...
                return (
                  <tr key={inv.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-3 font-bold text-slate-700">
                      {inv.invoice_no}
                      {inv.invoice_type === 'PURCHASE_RETURN' && <span className="bg-rose-100 text-rose-800 text-xs px-2 py-0.5 rounded ml-2 font-normal">RETURN</span>}
                    </td>
