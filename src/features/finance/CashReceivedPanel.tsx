...
export const CashReceivedPanel: React.FC = () => {
  ...
  const customerAccounts = accounts.filter(a => a.account_type_id !== 1); // Anyone except Cash itself
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(`Receipt of Rs. ${amount} saved successfully!`);
      setIsAddMode(false);
      setAmount(0);
      setDescription('');
      setAttachment(null);
